...
MODELS = ['teacher_training', 'wider_deeper_student',
          'wider_deeper_teacher', 'wider_student', 'wider_teacher']
...
def plot(metric_lists):
    _, ax = plt.subplots()
    for model, (epochs, metric) in zip(MODELS, metric_lists):
        ax.plot(epochs, metric, label=model)
    return ax
...
def main():
    ...
    plot_train_loss(train_loss_lists, dataset)
    plot_test_acc(test_acc_lists, dataset)
